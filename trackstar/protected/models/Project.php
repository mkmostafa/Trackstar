...
class Project extends TrackStarActiveRecord
{
	...
	{
		$owner=Yii::app()->user;
		$sql = "Insert Into tbl_project_user_assignment (project_id,user_id,create_time,create_user_id)
		     values(:project_id,:user_id,:create_time,:create_user_id)";
		$command=Yii::app()->db->createCommand($sql);
		$command->bindValue(':project_id',$this->id,PDO::PARAM_INT);
		$command->bindValue(':user_id',$user->id,PDO::PARAM_INT);
		$command->bindValue(':create_time',new CDbExpression('Now()'),PDO::PARAM_STR);
		$command->bindValue(':create_user_id',$owner->getId(),PDO::PARAM_INT);
		return $command->execute();
	}
