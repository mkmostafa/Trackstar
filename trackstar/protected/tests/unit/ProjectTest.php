...
class ProjectTest extends CDbTestCase
{
	...
	public function testUserRoleOptions()
	{
		$roles=Project::model()->getUserRoleOptions();
		$this->assertTrue(is_array($roles));
		$this->assertTrue(3==count($roles));
		$this->assertTrue(in_array('member',$roles));
	...
	{
		$project = $this->projects('project1');
		$project2 = $this->projects('project2');
		$user1=$this->users('user1');
		$user2=$this->users('user3');
		$this->assertTrue($project->isUserInProject($user1));
		$this->assertFalse($project2->isUserInProject($user2));
	}
